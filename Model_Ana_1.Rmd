...
## Case-crossover
```{r}
cross_mod <- function(rain = c(), city = c(), cause = "all", control.ratio = 1){
  ...
  control_subset <- subset(control_dates, doy %in% control_range)
  ...
  mod <- glm(new_df[, cause] ~ hurr + stratum + dow + year, 
             # Sunday is the reference day, and 1987 (no storm in 1987) is the reference year.
             # We may need an offset representing city population in the model
...
# example
a <- cross_mod(rain = "rain50", city = "miam", control.ratio = 5)
```
